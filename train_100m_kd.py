...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--data-glob", default=DATA_GLOB,
                   help="glob for training parquets; overrides DATA_GLOB. "
                        "Ignored when --data-files is set.")
    p.add_argument("--val-glob", default=VAL_GLOB,
                   help="glob for validation parquets; overrides VAL_GLOB.")
    # D4 (2026-05-02 quality-data push): explicit weighted file list,
    # generated by ``scripts/assemble_quality_corpus.py``. Format:
    #     PATH:WEIGHT,PATH:WEIGHT,...
    # The trainer parses this into ``files_with_weights=[(path, w), ...]``
    # and passes it to ``ParquetTokenStream``/``AsyncTokenStream``. When
    # set, OVERRIDES ``--data-glob``. Weights need not sum to 1.0 — they
    # are renormalised inside ``ParquetTokenStream.__init__``.
    p.add_argument("--data-files", default=None,
                   help="Comma-separated PATH:WEIGHT pairs for the "
                        "quality-corpus mixture (D4 of 2026-05-02 "
                        "token-soup root-cause hunt). Example: "
                        "'/data/fineweb_edu/000.parquet:0.5,"
                        "/data/wiki_zh/train-00000.parquet:0.3,"
                        "/data/sft_combined.jsonl:0.2'. "
                        "Generated by scripts/assemble_quality_corpus.py. "
                        "Overrides --data-glob when set.")
...
def main() -> int:
    ...
    # D4 (2026-05-02 quality-data push): parse --data-files into a
    # weighted file list. When set, it OVERRIDES --data-glob in the
    # train stream (val stream stays on --val-glob).  Format:
    #   PATH:WEIGHT,PATH:WEIGHT,...
    _files_with_weights: list[tuple[str, float]] | None = None
    if getattr(args, "data_files", None):
        _files_with_weights = []
        for part in str(args.data_files).split(","):
            part = part.strip()
            if not part:
                continue
            # rsplit on the LAST ':' so Windows-style paths or any colon
            # in the path don't break parsing. Drive-letter colons are
            # not in scope (rental is Linux) but rsplit makes us safe.
            if ":" not in part:
                raise ValueError(
                    f"--data-files entry {part!r} missing ':WEIGHT' "
                    f"suffix; expected PATH:WEIGHT"
                )
            ...
            try:
                ...
                raise ValueError(
                    f"--data-files entry {part!r}: weight {w!r} not "
                    f"a float ({exc})"
                ) from exc
            _files_with_weights.append((path, weight))
        if not _files_with_weights:
            _files_with_weights = None
        else:
            _log(f"[data-files] parsed {len(_files_with_weights)} files "
                 f"with weights from --data-files")
            for p, w in _files_with_weights:
                _log(f"[data-files]   {w:.4f}  {p}")
    _use_async_pipeline = bool(getattr(args, "async_data_pipeline", False))
    # When --data-files is set, --data-glob is unused for train but its
    # value is still passed (the wrapper requires a string); the inner
    # constructor ignores it.
    _train_glob = args.data_glob
    if _use_async_pipeline:
        from synapforge.data import AsyncTokenStream
        train_ds = AsyncTokenStream(
            _train_glob, seq_len=seq_len,
            ...
            files_with_weights=_files_with_weights,
        )
        ...
        train_ds = ParquetTokenStream(_train_glob, seq_len=seq_len,
                                      tokenizer_name=args.tokenizer_name,
                                      batch_size=args.batch_size, loop=True,
                                      shuffle_buffer=int(args.shuffle_buffer),
                                      shuffle_seed=int(args.shuffle_seed),
