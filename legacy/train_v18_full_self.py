...
def main():
    ...
    p.add_argument("--out", default="/workspace/runs/synapforge_v18_full_self")
    ...
    # ---- 9-modal random data builders (full-modal training) ----
    def make_image_batch(B): return torch.randn(B, 3, 32, 32, device=device)
    def make_audio_batch(B): return torch.randn(B, 16000, device=device)
    def make_video_batch(B): return torch.randn(B, 8, 3, 16, 16, device=device)
    def make_screen_batch(B): return torch.randn(B, 3, 64, 64, device=device)
    def make_pc_batch(B): return torch.randn(B, 256, 6, device=device)
    def make_ts_batch(B): return torch.randn(B, 64, 8, device=device)
    def make_graph_batch(B):
        return {"nodes": torch.randn(B, 16, 32, device=device),
                "edges": torch.zeros(B, 16, 16, device=device, dtype=torch.long)}
    def make_bio_batch(B): return torch.randn(B, 1024, 8, device=device)
    modal_specs = [
        ("image", "head_image", make_image_batch),
        ("audio", "head_audio", make_audio_batch),
        ("video", "head_video", make_video_batch),
        ("screen", "head_screen", make_screen_batch),
        ("point_cloud", "head_pc", make_pc_batch),
        ("time_series", "head_ts", make_ts_batch),
        ("graph", "head_graph", make_graph_batch),
        ("biosignal", "head_bio", make_bio_batch),
    ]
    try:
        from synapforge.self_learn import ExperienceReplayBuffer
        replay_buf = ExperienceReplayBuffer(capacity=512, alpha=0.5)
        if _DDP_RANK == 0: _log("ExperienceReplayBuffer wired")
    except Exception as e:
        replay_buf = None
    ...
    for step in range(1, args.steps + 1):
        ...
        with torch.amp.autocast(device_type=device.type, dtype=DTYPE,
            ...
            # 9-modal recon (rotates one modality per 10 steps)
            modal_l = torch.zeros((), device=device, dtype=ce.dtype)
            mp = step % 10
            if mp < 8:
                mname, hattr, bfn = modal_specs[mp]
                m_input = bfn(8)
                m_h = base_model.encode_modal({mname: m_input})
                pred = getattr(base_model, hattr)(m_h.mean(dim=1)).float()
                # MSE target = first elements of input
                if isinstance(m_input, dict):
                    tgt = m_input["nodes"].mean(dim=1).float()
                else:
                    tgt = m_input.reshape(m_input.shape[0], -1).float()[:, :pred.shape[-1]]
                modal_l = F.mse_loss(pred[:, :tgt.shape[-1]], tgt[:, :pred.shape[-1]])
                loss = loss + 0.05 * modal_l
        ...
        # Idle exploration: model self-rolls a Q: prompt every 200 steps
        if step % 200 == 0 and _DDP_RANK == 0:
            try:
                with torch.no_grad():
                    base_model.eval()
                    ids = torch.tensor([student_tok.encode("Q:")], dtype=torch.long, device=device)
                    for _ in range(24):
                        with torch.amp.autocast(device_type=device.type, dtype=DTYPE,
                                                enabled=device.type == "cuda"):
                            h_ = base_model.encode_text(ids)
                            logits_ = base_model.lm_logits(h_[:, -1:, :]).float()
                        topk = torch.topk(logits_[0, -1], k=40)
                        probs = torch.softmax(topk.values / 0.8, dim=-1)
                        nxt = topk.indices[torch.multinomial(probs, 1)]
                        ids = torch.cat([ids, nxt.view(1, 1)], dim=1)
                        if nxt.item() == (student_tok.eos_token_id or 50256): break
                    base_model.train()
                    text = student_tok.decode(ids[0].tolist())
                    _log(f"  IDLE step {step}: {text[:120]!r}")
                    if replay_buf is not None and len(text) > 10:
                        try: replay_buf.add(("idle", ids[0].cpu().tolist()), priority=1.0)
                        except Exception: pass
            except Exception as e:
                _log(f"  IDLE failed: {e}")
        ...
        if step % 10 == 0 or step == 1:
            ...
            _log(f"step {step:5d} loss={loss.item():.4f} ce={ce.item():.3f} "
                 ...
                 f"def={def_l.item():.3f} mod={modal_l.item():.3f} z={z.item():.1f} lr={cur_lr:.5f} "
